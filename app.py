...
def convene():
    ...
    def generate():
        ...
        while len(active_members) > 1:
            # --- ROUND START ---
            yield sse_event("round_start", {"round": round_num, "survivors": active_members})
            time.sleep(0.3)
            ...
            for member in active_members:
                ...
                yield sse_event("member_answered", {"member": member, "answer": answer_text})
                time.sleep(0.2)
            ...
            loser, reasoning = arbiter_eliminate(question, current_answers, votes, map_data)
            yield sse_event("arbiter_decision", {"reasoning": reasoning, "round": round_num})
            time.sleep(0.5)
